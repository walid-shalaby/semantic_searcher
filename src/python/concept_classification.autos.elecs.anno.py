...
if mode in ('max_multi_thread_resolved_cosines','hungarian_multi_thread_resolved_cosines','hungarian_spark_resolved_cosines'):
    ...
    output_path_hdfs = 'samples-out-concepts-conc-allnorm-0.65.hung.anno.resolved.autos.elecs.txt'
...
def main():
    ...
    if mode=='cosine':
        ...
        needed_cosines = all_cosines.filter(lambda line: len(line)>1).flatMap(lambda line: load_cosines_spark(line,min_cosine=0.65)).filter(lambda line: line is not None)
        titles_cosines = {}
