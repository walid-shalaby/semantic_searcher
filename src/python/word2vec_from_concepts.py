...
concepts_path = '/home/walid/work/github/semantic_searcher/enwiki-concepts-20160820-no-redirects-dic.txt'
concepts_path = '/scratch/wshalaby/EntityTypeRecognition/etr/data/data-prep/enwiki-concepts-20160820-no-redirects-dic.txt'
concepts_path = '/scratch/wshalaby/EntityTypeRecognition/etr/data/data-prep/enwiki-concepts-raw-20160820-no-redirects-dic.txt'
concepts_path = '/scratch/wshalaby/EntityTypeRecognition/etr/data/data-prep/enwiki-concepts-self-20160820-no-redirects-dic.txt'
concepts_path = '/home/walid/work/github/semantic_searcher/enwiki-concepts-self-20160820-no-redirects-dic.txt'

class MySentences(object):
    global titles
    def __init__(self, dirname, mode='no-redirects-do-all'):
        ...
        if self.mode=='no-redirects':
            ...
            for line in open(self.dirname):
                ...
                for items in col:                   
                    ...
                    if len(tokens)==1: # @wshalaby, if a line contains only 1 mention, ignore it
                        continue                
                    for item in tokens:
                        if len(item)>0:
                            ...
                            if item in titles:
                                ...
                                print('|'+item+'|')                                                    
                    # @wshalaby, return sentence per line not for all article
                    yield outcol
                #yield outcol
        elif self.mode=='no-redirects-do-all':
            #for fname in os.listdir(self.dirname):
            #for line in open(os.path.join(self.dirname, fname)):
            for line in open(self.dirname):
                if line.find('File:')==0:
...
titles, redirects, seealso = load_titles(titles_path, to_keep, True,'title')
...
sys.stdout.flush()
for sg in [1]:#[1,0]:
    for size in [500,50,100,200,300,400]:
        for window in [1,2,3,4,6,7,8,9,10]:
            for iter in [10]:#[1,0]:
                ...
                if sg==1: #skip-gram
                    outpath = '/media/wshalaby/b667e28e-5e90-4884-8810-5d897c9e56ce/work/github/semantic_searcher/concepts-'+str(iter)+'iter-dim'+str(size)+'-wind'+str(window)+'-skipgram'+str(sg)+'.model'
                    outpath = '/home/walid/work/github/semantic_searcher/concepts-'+str(iter)+'iter-dim'+str(size)+'-wind'+str(window)+'-skipgram'+str(sg)+'.model'
                    outpath = '/scratch/wshalaby/doc2vec/concepts'+str(iter)+'iter-dim'+str(size)+'-wind'+str(window)+'-skipgram'+str(sg)+'.model'
                    outpath = '/scratch/wshalaby/doc2vec/concepts-raw-'+str(iter)+'iter-dim'+str(size)+'-wind'+str(window)+'-skipgram'+str(sg)+'.model'
                    outpath = '/scratch/wshalaby/doc2vec/concepts-self-dic-'+str(iter)+'iter-dim'+str(size)+'-wind'+str(window)+'-skipgram'+str(sg)+'.model'
                    outpath = '/home/walid/work/github/semantic_searcher/concepts-self-dic-'+str(iter)+'iter-dim'+str(size)+'-wind'+str(window)+'-skipgram'+str(sg)+'.model'
                    print("training "+strftime("%Y-%m-%d %H:%M:%S", gmtime()))
                    sys.stdout.flush()
                    model = gensim.models.Word2Vec(sentences, min_count=1, iter=iter, window=window, size=size, workers=multiprocessing.cpu_count()-1, sg=sg)
                    print("saving: ("+outpath+')'+strftime("%Y-%m-%d %H:%M:%S", gmtime()))
                    model.save(outpath)
                    sys.stdout.flush()
                else: # cbow
                    for cbow_mean in [1,0]:
                        outpath = '/media/wshalaby/b667e28e-5e90-4884-8810-5d897c9e56ce/work/github/semantic_searcher/concepts-10iter-dim'+str(size)+'-wind'+str(window)+'-skipgram'+str(sg)+'-cbow_mean'+str(cbow_mean)+'.model'
                        model = gensim.models.Word2Vec(sentences, min_count=1, iter=10, window=window, size=size, workers=multiprocessing.cpu_count()-1, sg=sg, cbow_mean=cbow_mean)
                        print("saving: ("+outpath+')'+strftime("%Y-%m-%d %H:%M:%S", gmtime()))
                        model.save(outpath)
        break
print('Done!')
